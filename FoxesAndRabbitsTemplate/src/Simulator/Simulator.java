...
public class Simulator {
    ...
    private static final double DINOSAUR_CREATION_PROBABILITY = 0.02;
    ...
    public Simulator(int width, int height) {
        ...
        DinosaurList = new ArrayList<Dinosaur>();
    ...
    public void setGUI(PApplet p) {
        ...
        view.setColor(Dinosaur.class, p.color(200,0,0));
        ...
        graph.title = "Animals.Fox and Animals.Rabbit Populations and Animals.Dinosaur populations";
        graph.xlabel = "Time";
        graph.ylabel = "Pop.\t\t";
        graph.setColor(Rabbit.class, p.color(155, 155, 155));
        graph.setColor(Fox.class, p.color(200, 0, 255));
        graph.setColor(Dinosaur.class, p.color(200,200,200));
    }
    ...
    public void simulateOneStep() {
        ...
        ArrayList<Dinosaur> babyDinosaurStorage = new ArrayList<Dinosaur>();
        for (int i = 0; i < DinosaurList.size(); i++) {
            Dinosaur dinosaur = DinosaurList.get(i);
            dinosaur.hunt(field, updatedField, babyDinosaurStorage);
            if (!dinosaur.isAlive()) {
                DinosaurList.remove(i);
                i--;
            }
        }

        DinosaurList.addAll(babyDinosaurStorage);
    ...
    private void initializeBoard(Field field) {
        ...
        for (int row = 0; row < field.getHeight(); row++) {
            for (int col = 0; col < field.getWidth(); col++) {
                ...
                } else if (rand.nextDouble() <= DINOSAUR_CREATION_PROBABILITY) {
                    Dinosaur dinosaur = new Dinosaur(true);
                        dinosaur.setLocation(row,col);
                        DinosaurList.add(dinosaur);
                        field.put(dinosaur,row,col);
                }
            }
        }
        Collections.shuffle(rabbitList);
        Collections.shuffle(foxList);
        Collections.shuffle(DinosaurList);
    }
            ...
            for (int y = loc.getRow() - 8; y < loc.getRow() + 8; y++) {
                ...
                if (field.isLegalLocation(locToCheck)) {
                    ...
                    if( animal instanceof  Dinosaur)
                        DinosaurList.remove((Dinosaur) animal);
