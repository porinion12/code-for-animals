...
public class Dinosaur {
    ...
    private static double BREEDING_PROBABILITY = 1;
    // The maximum number of births.
    private static int MAX_LITTER_SIZE = 50;
     ...
     */
    public Dinosaur(boolean startWithRandomAge) {
        age = 0;
        alive = true;
        if (startWithRandomAge) {
            age = (int)(Math.random()*MAX_AGE);
            foodLevel = (int)(Math.random()*RABBIT_FOOD_VALUE);
            foodlevel2 = (int)(Math.random()*FOX_FOOD_VALUE);

        } else {
            // leave age at 0
            foodLevel = RABBIT_FOOD_VALUE;
            foodlevel2 = FOX_FOOD_VALUE;
        }
    }
     ...
     */
    public void hunt(Field currentField, Field updatedField, List<Dinosaur> babyFoxStorage) {
        incrementAge();
        incrementHunger();
        if (alive) {
            ...
            for (int b = 0; b < births; b++) {
                Dinosaur newDinosaur = new Dinosaur(false);
                newDinosaur.setFoodLevel(this.foodLevel);
                newDinosaur.setFoodLevel(this.foodlevel2);
                babyFoxStorage.add(newDinosaur);
                Location loc = updatedField.randomAdjacentLocation(location);
                newDinosaur.setLocation(loc);
                updatedField.put(newDinosaur, loc);
            }
